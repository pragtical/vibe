...
core._G__index = getmetatable(_G).__index
...
local function enable_strict_global()
  getmetatable(_G).__index = core._G__index
...
function misc.exec_with_requires(text)
...
function string:last_WORD(sep)
  sep = sep or ' '
  local last_WORD = self:find("[^"..sep.."]+$")
  if last_WORD==nil or last_WORD==#self then
    return nil
  end
...
function misc.predict_exec_input(text, item)
  ...
  if dot_pos == nil then
    ...
    if misc.is_table(metatable) then
      r = table.map(
        table.keys(metatable),
        function(item)
          return text:sub(1,last_WORD_start-1)..pre_dot..last_WORD:sub(dot_pos, dot_pos)..item
        end
      )
    end
  end
...
function table.list_concat(a, b,...)
  if b == nil then
    return a
  end
  local R = table.copy(a)
  for _, e in ipairs(table.list_concat(b,...)) do
    table.insert(R, e)
  end
  return R
end
